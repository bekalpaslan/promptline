...
`tone` says what the chip is: `neutral` (a plain label), `muted` (a count or note), `tag` (a #tag's hue, via `hue`; `active` fills it), `builtin` / `field` / `config` / `bad` (the four placeholder kinds), `warn` (the fill-in count), `primary` (a pack). `size` says where it sits: `sm` is 16px, in a row or card; `md` is 20px, level with an input in the editor; `inline` sits inside wrapping text and breaks with it.
